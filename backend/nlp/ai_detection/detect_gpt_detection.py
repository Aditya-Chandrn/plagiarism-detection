...
def generate_perturbations(text, num_perturbations=5):
    ...
    for _ in range(num_perturbations):
        perturbed_words = [
            random.choice(synonym_dict.get(
                word, [word])) if word in synonym_dict and random.random() < 0.3 else word
            for word in words
        ]
...
def detect_ai_generated_text(text, num_perturbations=5, threshold=0.5):
    ...
    # Step 5: Calculate average log probability of perturbed texts
    avg_perturbed_log_prob = sum(
        perturbed_log_probs) / len(perturbed_log_probs)

    # Step 6: Calculate curvature score
    curvature_score = original_log_prob - avg_perturbed_log_prob

    # Step 7: Classify based on curvature score
